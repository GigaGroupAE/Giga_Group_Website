...
const AboutProject = ({ amenitiesDesc, amenitiesTitle, features }) => {
  ...
  return (
    <div className="relative px-1 md:px-4 overflow-hidden">
      ...
      <div className="relative mx-auto  items-center justify-center flex z-10">
        <motion.div
          ref={ref}
          initial="hidden"
          animate={isInView ? 'visible' : 'hidden'}
          variants={containerVariants}
          className="  backdrop-blur-sm px-6 py-14"
        >
          <motion.h2
            variants={cardVariants}
            className="text-start md:text-center text-3xl xl:text-4xl font-poppins text-[#2D2D2D]"
          >
            {amenitiesTitle}
          </motion.h2>
          ...
          <motion.div
            variants={containerVariants}
            className="grid py-12  items-center justify-center grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4"
          >
            {features.map((feature, index) => (
              ...
              >
                <div className="text-yellow-500 text-2xl mb-2">{feature.icon}</div>
                <p className="text-sm text-center font-poppins text-[#2D2D2D]">{feature.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
